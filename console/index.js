...
let IGNORE_DIRS = [];
try {
  const fileContent = fs.readFileSync("console/ignore-items.txt", "utf-8");
...
\x1b[0;33mnode console . c\x1b[0m               Print full project tree in console
\x1b[0;33mnode console . s\x1b[0m               Save full project tree to tree.txt
\x1b[0;33mnode console <foldername> c\x1b[0m    Print only <foldername> folder tree
\x1b[0;33mnode console <foldername> s\x1b[0m    Save only <foldername> tree to tree.txt
`;
...
if (mode === "c") {
  ...
  // Create structure folder if it doesn't exist
  const structureDir = path.join(root, "structure");
  if (!fs.existsSync(structureDir)) {
    fs.mkdirSync(structureDir, { recursive: true });
  }
  
  fs.writeFileSync(path.join(structureDir, "tree.txt"), output, { encoding: "utf8" });
  console.log("\x1b[1;32mSaved directory tree to structure/tree.txt\x1b[0m");
} else {
